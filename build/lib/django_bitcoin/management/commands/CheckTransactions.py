    ...
    def handle_noargs(self, **options):
        ...
        while time() - start_time < float(RUN_TIME_SECONDS):
            ...
            for t in transactions:
                if t['category'] != 'immature' and (not last_check_time or (int(t['time'])) >= last_check_time) and t['amount']>0:
                    dps = DepositTransaction.objects.filter(txid=t['txid'])
                    if dps.count() == 0:
                        try:
                            ba = BitcoinAddress.objects.get(address=t['address'], active=True, wallet__isnull=False)
                            if ba:
                                ba.query_bitcoind(0, triggered_tx=t['txid'])
                            last_check_time = int(t['time'])
                        except BitcoinAddress.DoesNotExist:
                            pass
                    elif Decimal(str(t['amount'])) == dps[0].amount and int(t['confirmations'])>dps[0].confirmations and dps.count()==1:
                        dp = dps[0]
                        DepositTransaction.objects.filter(id=dp.id).update(confirmations=int(t['confirmations']))
                        if int(t['confirmations']) >= settings.BITCOIN_MINIMUM_CONFIRMATIONS:
                            dp.address.query_bitcoind(triggered_tx=t['txid'])
