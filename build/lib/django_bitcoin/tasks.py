...
def query_transactions():
    with NonBlockingCacheLock("query_transactions_ongoing"):
        ...
        for tx in transactions:
            ba = BitcoinAddress.objects.filter(address=tx['address'])
            if ba.count() > 1:
                raise Exception("Too many addresses!")
            if ba.count() == 0:
                print("no address found, address", tx['address'])
                continue
            ba = ba[0]
            dps = DepositTransaction.objects.filter(txid=tx['txid'], amount=tx['amount'], address=ba)
            if dps.count() > 1:
                raise Exception("Too many deposittransactions for the same ID!")
            elif dps.count() == 0:
